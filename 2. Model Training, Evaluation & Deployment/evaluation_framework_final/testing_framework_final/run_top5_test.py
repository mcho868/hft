#!/usr/bin/env python3
"""
Runner script for TOP 5 Medical Triage Configuration Testing

This script tests the top 5 performing configurations on the full test dataset.
...
Usage:
  python run_top5_test.py              # Full test on all 1975 cases
  python run_top5_test.py --test       # Test run on 1 configuration
  python run_top5_test.py --resume     # Resume from latest progress
"""
...
def main():
    ...
    logger.info("🏆 TOP 5 MEDICAL TRIAGE CONFIGURATION TESTING")
    logger.info("=" * 60)
    ...
    # Import the tester
    try:
        from comprehensive_triage_tester import Top5MedicalTriageTester
    except ImportError as e:
        logger.error(f"❌ Failed to import tester: {e}")
    ...
    if len(sys.argv) > 1:
        ...
        if mode == "--test":
            ...
            logger.info("🔄 Running RESUME MODE")
            ...
            logger.info("🚀 Running FULL MODE (all 5 configurations)")
            results = tester.run_comprehensive_evaluation()
    else:
        logger.info("🚀 Running FULL MODE (all 5 configurations)")
        results = tester.run_comprehensive_evaluation()
    ...
    logger.info("=" * 60)
    logger.info("🎉 TOP 5 TESTING COMPLETED!")
